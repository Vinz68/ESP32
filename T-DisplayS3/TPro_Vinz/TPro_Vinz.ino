...
const char* ssid     = "MyWifiNetwork";   // <======     set your SSID here
const char* password = "MyPassword";      // <======     set your network password here
uint8_t broadcastAddress[] = {0x00, 0x00, 0x00, 0x00, 0x64, 0x00}; 
String ip=""; 

const char* ntpServer = "pool.ntp.org";
const long  gmtOffset_sec =3600;          // time zone * 3600 , my time zone is  +1 GMT
...
{
  Serial.println();
  Serial.println("GetTime, starting....");  
  ...
  Serial.println("Get timeinfo struct");
  configTime(gmtOffset_sec, daylightOffset_sec, ntpServer);
  struct tm timeinfo;
  if (getLocalTime(&timeinfo))
  {
    Serial.println("SetTimeStruct");
    rtc.setTimeStruct(timeinfo); 
  }

  Serial.println("GetTime finished.");    
}
...
unsigned long timePasedHomeKey=0;
void touchHomeKeyCallback(void *user_data)
{
  if(timePasedHomeKey+updatePeriod<millis())
  {
      Serial.println("HomeKeyCallback."); 

      timePasedHomeKey=millis();

      //TODO, react on HomeKey Pressed, do something usefull....
  }
}
...
void setup()
{
  ...
  Serial.println("Setup initializing (connect wifi)."); 
  connectWifi();

  Serial.println("Setup initializing (get time)."); 
  getTime();
  ip=WiFi.localIP().toString();

  Serial.println("Setup initializing (disconnect wifi).");
  delay(20);
  WiFi.disconnect();
  WiFi.mode(WIFI_STA);

  Serial.println("Setup initializing (init ESP-NOW).."); 
  // Init ESP-NOW
  ...
  Serial.println("Setup initializing (Register &  Add peer).."); 
  // Register peer
  ...
  Serial.println("Setup initializing (PMU.init).."); 
  PMU.init(Wire, BOARD_I2C_SDA, BOARD_I2C_SCL, SY6970_SLAVE_ADDRESS);
  PMU.enableADCMeasure();

  Serial.println("Setup initializing (SensorLTR553 - als).."); 
  ...
  Serial.println("Setup initializing (Screen - white to black).."); 
  int co=225;
  for(int i=0;i<15;i++)
  {
  ...
  setBrightness(BRIGHTNESS_SETUP_LEVEL);
...
void loop()
{
  ...
  if (touched) {
    ...
    {
      barValue=map(y[0],370,160,0,BRIGHTNESS_MAX_LEVEL);
